...
class Voter{
 public :
  ...
  virtual float vote(vector<float> label,
		     double ** score_matrix){}  /* for an n*n(n is the num of label) *full* score matrix
						 * get the result ot the test data
						 */

  virtual float vote(vector<float> label,
		    double ** score_matrix,
		    double & confident){} /* the confident of the label */
  
  virtual void matrix_inverse(vector<float> label,
			      double ** score_matrix){} /* I only give you a *up_triangle* score matrix
